...
<html>
    <head>
        ...
        <title>Ajouter son service</title>
     ...
     <body>
        ...
        <div class="body">
            ...
            ?>
            <h1 style="margin-left:10px;">Ajoutez le service que vous souhaitez proposer</h1>
...
<?php
    ...
    else
    {
        ...
        if($estPersonne === false)
        {
            ...
            //var_dump($infosAdrr);
                if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST))
                {
                    ...
                    $managerInfosPersonne->verifierEtInscrireAdrrPers($idMembre);
                }
            echo '</div>';
        ...
        else 
        {
            ...
            $infosService = $managerOffre->infoAvantInsertServicePropose($idMembre);
            if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST))
            {
                echo '<div class="validation">';         
                //$infosService = $managerOffre->infosServiceProposeAffichable($idMembre);
               //var_dump($infosService); 
                $infosService = $managerOffre->verifierEtInsertOffreService($idMembre);
                echo '</div>';     
            }


            include('formulaireAjoutService.php');
        }
    }
?>
